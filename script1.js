...
function resetAllStats() {
   ...
   teamPointsElements1.textContent = 'Team 1 Average Placement: 0';


   const teamPointsElements2 = document.getElementById('team2Points');
       teamPointsElements2.textContent = 'Team 2 Average Placement: 0';


       const teamPointsElements3 = document.getElementById('team3Points');
           teamPointsElements3.textContent = 'Team 3 Average Placement: 0';


           const teamPointsElements4 = document.getElementById('team4Points');
               teamPointsElements4.textContent = 'Team 4 Average Placement: 0';


               const teamPointsElements5 = document.getElementById('team5Points');
                   teamPointsElements5.textContent = 'Team 5 Average Placement: 0';


                   const teamPointsElements6 = document.getElementById('team6Points');
                       teamPointsElements6.textContent = 'Team 6 Average Placement: 0';


                       const teamPointsElements7 = document.getElementById('team7Points');
                           teamPointsElements7.textContent = 'Team 7 Average Placement: 0';


                           const teamPointsElements8 = document.getElementById('team8Points');
                               teamPointsElements8.textContent = 'Team 8 Average Placement: 0';
            ...
            data.stats.forEach(stat => {
                if (option === '1') {
                    if (stat.key.includes('1') && !stat.key.includes('nc')) {
                        const  points  = stat.value.placement;
                        console.log("Data:" + JSON.stringify(stat.value.placement));
   ...
   for (let i = 1; i <= 5; i++) {
       ...
       fetchPlayerStats(playerName)
           .then(statsWithPlacement => {
               ...
               teamPointsElement.textContent = `Team ${teamId} Average Placement: ${teamPoints/5}`;
           })
           .catch(error => {
   ...
   btn.addEventListener('click', () => {
       ...
       fetchPlayerStats(playerName)
           .then(statsWithPlacement => {
              ...
              playerStatsElement.textContent = `Player ${playerName}: Average Placement(${statsWithPlacement.points})`;
...
function compareAllTeams() {
    ...
    setTimeout(() => {
        ...
        let message = '<h2>Teams Ranking by Average Placements</h2>';
        teams.forEach(team => {
            message += `<h3>Team ${team.teamId}: ${team.points} Average Placement</h3>`;
